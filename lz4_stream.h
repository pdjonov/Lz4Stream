...
/*
	...
	1.	Allocate an instance of lz4_dec_stream_state (anywhere,
		on the stack if you like) and call lz4_dec_steram_init
	...
	4.	Call lz4_dec_steram_run. It will decode as much as it
		can before it either runs out of input data or fills
		the entire output buffer. If lz4_dec_steram_run returns
	...
	6.	Deallocate the lz4_dec_stream_state object. If you wish
		to reuse it, simply call lz4_dec_steram_init again. It
		holds no external resources, so it's safe to deal with
		it any way you like.

	lz4_dec_steram_run updates the in, avail_in, out, and avail_out
